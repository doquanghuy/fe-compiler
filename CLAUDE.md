# CLAUDE.md — fe-compiler-v1 agent context

## Repo identity

`fe-compiler-v1` is a **frontend-domain compiler package**
consumed by `axcore-v1` via the `axcore.plugins` entry-point
group. It is not a framework; it ships plugin + workflow + one
step bundle.

Plugin id: **`fe`**. Importable: `fe_compiler`. Distribution:
`fe-compiler-v1`. Sibling: `be-compiler-v1` (plugin id `be`).
Both depend on `axcore-v1` and import only from `axcore.api`.
...
[`../axcore-v1/docs/audit.md`](../axcore-v1/docs/audit.md). Add
...
1. **This repo is a plugin/domain package.** It *consumes* the
   `axcore-v1` framework; it does not re-implement it. Primary
   shell is **Spec Kit**.
...
6. **No phase / prompt / sprint identifiers anywhere in code,
...
- Depend on `be-compiler-v1`.
- Install `axcore-v1` as a git dependency.
